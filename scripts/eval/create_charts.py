...
class EvaluationComparer:
    ...
    def read_and_extract_score(self, filepath):
        with open(filepath, 'r') as file:
            ...
            if self.score_type not in data or self.score not in data[self.score_type]:
                return 0, False
            return data[self.score_type][self.score], True
    ...
    def collect_data(self):
        for filepath in Path(self.eval_output_dir).rglob('*.json'):
            ...
            f1_score, is_valid = self.read_and_extract_score(filepath)

            if not is_valid:
                continue

            self.scores_by_task.setdefault(task, {}).setdefault(dataset, {})[model] = f1_score
    ...
    def plot_by_dataset(self, datasets, datasets_scores, models):
        if len(datasets) > 1:
            ...
            plt.xlabel(f"Models [Dataset: {datasets[0]}]")

    def format_plot(self, task, datasets):
        title_task, title_score_type, title_score = self.format_title_components(task)
        plt.title(f"{title_task}: {title_score_type} {title_score}")
        plt.ylabel(title_score)
        plt.ylim(0, 1)
        if len(datasets) > 1:
...
def main():
    ...
    parser.add_argument("-o", "--output_dir", type=str, default='charts', help='The directory to save the charts. Default: charts')
