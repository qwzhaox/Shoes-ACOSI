...
class EvalChartGenerator:
    def __init__(self, scores, eval_output_dir, output_dir, ignore_llama):
    ...
    def collect_data(self):
        for filepath in Path(self.eval_output_dir).rglob('*.json'):
            ...
            metadata = self.__parse_filepath(filepath)

            model = self.__get_formmated_model(metadata['model_type'], metadata['model'])
            dataset = self.__get_formmated_dataset(metadata['dataset'], metadata['task'])

            self.models.add(model)
            if "Rest" in dataset:
                dataset = dataset.replace("Rest", "Restaurant")
            task = metadata['task']
            score_dict, is_valid = self.__read_and_extract_scores(filepath)

            if not is_valid:
                continue

            self.scores_by_task.setdefault(task, {}).setdefault(dataset, {})[model] = score_dict
    ...
    def __get_formmated_model(self, model_type, model):
        return f"{model_type.upper()}: {model.upper()}"

    def __get_formmated_dataset(self, dataset, task):
        return f"{dataset.title()}-{task.split('-')[0].upper()}"

    def __read_and_extract_scores(self, filepath):
        with open(filepath, 'r') as file:
            data = json.load(file)
            for score in self.scores:
                if score not in data or self.score not in data[score]:
...
def main():
    ...
    parser.add_argument('-st', '--scores', type=list, default=['exact'], help='The type of scores to compare.')
    parser.add_argument('-i', '--ignore_llama', action='store_true', help='Ignore llama models in the comparison.')

    args = parser.parse_args()

    comparer = EvalChartGenerator(args.scores, args.eval_output_dir, args.output_dir, args.ignore_llama)
